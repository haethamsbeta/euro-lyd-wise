...
export const groupsApi = {
  list: (params: { q?: string } = {}) =>
    apiFetch<HolderGroup[]>(`/api/groups${qs(params)}`),
  get: (id: string) => apiFetch<HolderGroup>(`/api/groups/${id}`),
  members: (id: string) =>
    apiFetch<Array<{ holder_id: string; holder_name: string; added_at: string }>>(
      `/api/groups/${id}/members`,
    ),
  addMember: (id: string, holder_id: string) =>
    apiFetch<{ ok: true }>(`/api/groups/${id}/members`, {
      method: "POST",
      body: JSON.stringify({ holder_id }),
    }),
  removeMember: (id: string, holder_id: string) =>
    apiFetch<{ ok: true }>(`/api/groups/${id}/members/${holder_id}`, { method: "DELETE" }),
  create: (body: { name: string; description?: string }) =>
    apiFetch<HolderGroup>("/api/groups", { method: "POST", body: JSON.stringify(body) }),
};
