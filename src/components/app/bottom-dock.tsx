  ...
  return (
    <Link
      ...
      className={cn(
        ...
        "min-w-[64px] min-h-[52px] px-3 py-2 sm:min-w-[56px] sm:min-h-[44px] sm:px-3 sm:py-1.5",
        active
          ? "text-gold"
    ...
    >
      ...
      <div className="relative">
        <Icon className="w-6 h-6 sm:w-5 sm:h-5" strokeWidth={1.75} />
      ...
      <span className="text-[11px] sm:text-[10px] font-medium tracking-wide truncate max-w-[72px]">{label}</span>
    </Link>
  );
}
...
function Fab({ label }: { label: string }) {
  return (
    <Link
      ...
      aria-label={label}
      title={label}
      className="group relative inline-flex items-center justify-center rounded-full border-2 border-background bg-gradient-gold text-primary-foreground transition-all -mt-10 mx-3 w-[72px] h-[72px] sm:-mt-8 sm:mx-2 sm:w-16 sm:h-16 lg:-mt-7 lg:w-14 lg:h-14"
      style={{
        boxShadow:
    ...
    >
      ...
      <Plus
        className="relative z-10 w-9 h-9 sm:w-7 sm:h-7 transition-transform duration-300 group-hover:rotate-90"
        strokeWidth={2.5}
      />
    </Link>
  );
...
export function BottomDock() {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative pointer-events-auto mx-3 mb-2 max-w-2xl h-[76px] sm:mx-4 sm:mb-3 sm:h-16 lg:mx-auto rounded-2xl bg-card/90 backdrop-blur-xl border border-gold/20 flex items-center justify-around px-2"
        style={{
          boxShadow:
